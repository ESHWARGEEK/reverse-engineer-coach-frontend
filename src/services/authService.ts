import { RegisterData, User } from '../store/authStore';
...
class AuthService {
  ...
  async testApiKeys(token: string): Promise<ApiKeyTestResult> {
    const response = await fetch(`${API_BASE_URL}/api/v1/auth/api-keys/test`, {
      headers: this.getAuthHeaders(token),
    });

    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(errorData.detail || 'Failed to test API keys');
    }
