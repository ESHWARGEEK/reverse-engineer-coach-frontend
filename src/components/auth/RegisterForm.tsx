...
import { Eye, EyeOff, Mail, Lock, AlertCircle, Key, ChevronDown, ChevronUp, CheckCircle, Loader2 } from 'lucide-react';
...
export const RegisterForm: React.FC<RegisterFormProps> = ({ onSwitchToLogin, onSuccess }) => {
  ...
  const [apiKeyValidation, setApiKeyValidation] = useState<{
    github: { isValidating: boolean; isValid: boolean | null; error?: string };
    openai: { isValidating: boolean; isValid: boolean | null; error?: string };
    gemini: { isValidating: boolean; isValid: boolean | null; error?: string };
  }>({
    github: { isValidating: false, isValid: null },
    openai: { isValidating: false, isValid: null },
    gemini: { isValidating: false, isValid: null },
  });

  // Debounce timer for API key validation
  ...
  useEffect(() => {
    if (formData.password) {
      ...
      if (!validation.isValid) {
        setValidationErrors(prev => ({ ...prev, password: 'Password does not meet requirements' }));
      } else {
        setValidationErrors(prev => {
          const newErrors = { ...prev };
  ...
  const validateApiKey = useCallback(async (key: string, type: 'github' | 'openai' | 'gemini') => {
    if (!key.trim()) {
      setApiKeyValidation(prev => ({
        ...prev,
        [type]: { isValidating: false, isValid: null }
      }));
      return;
    }

    // Basic format validation first
    const isValidFormat = authService.validateApiKey(key, type);
    if (!isValidFormat) {
      setApiKeyValidation(prev => ({
        ...prev,
        [type]: { isValidating: false, isValid: false, error: `Invalid ${type} API key format` }
      }));
      return;
    }
    ...
    setApiKeyValidation(prev => ({
      ...prev,
      [type]: { isValidating: true, isValid: null }
    }));

    try {
      // Test the API key with actual API call
      const testResult = await testApiKeyWithService(key, type);
      setApiKeyValidation(prev => ({
        ...prev,
        [type]: { 
          isValidating: false, 
      ...
      setApiKeyValidation(prev => ({
        ...
        [type]: { 
          isValidating: false, 
          isValid: false, 
          error: `Failed to validate ${type} API key` 
        }
      }));
    }
  }, []);

  // Test API key with actual service
  const testApiKeyWithService = async (key: string, type: 'github' | 'openai' | 'gemini'): Promise<{ isValid: boolean; error?: string }> => {
    try {
      const response = await fetch('/api/v1/auth/test-api-key', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        return { isValid: false, error: errorData.detail || 'API key validation failed' };
      }

      const result = await response.json();
      return { isValid: result.valid, error: result.error };
    } catch (error) {
      return { isValid: false, error: 'Network error during API key validation' };
    }
  };

  // Debounced API key validation
  const handleApiKeyChange = useCallback((type: 'github' | 'openai' | 'gemini', value: string) => {
    const fieldMap = {
      github: 'githubToken',
      openai: 'openaiApiKey',
      gemini: 'geminiApiKey'
    ...
    updateFormData(fieldMap[type], value);

    // Clear existing timer
    if (validationTimers[type]) {
      clearTimeout(validationTimers[type]);
    }

    // Set new timer for validation
    const timer = setTimeout(() => {
      validateApiKey(value, type);
    }, 1000); // 1 second debounce

    setValidationTimers(prev => ({ ...prev, [type]: timer }));
  }, [updateFormData, validateApiKey, validationTimers]);
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.preferredAiProvider === 'openai') {
      if (!formData.openaiApiKey) {
        errors.openaiApiKey = 'OpenAI API key is required when OpenAI is selected';
      } else if (apiKeyValidation.openai.isValid === false) {
        errors.openaiApiKey = apiKeyValidation.openai.error || 'Invalid OpenAI API key';
      }
    }

    if (formData.preferredAiProvider === 'gemini') {
      if (!formData.geminiApiKey) {
        errors.geminiApiKey = 'Gemini API key is required when Gemini is selected';
      } else if (apiKeyValidation.gemini.isValid === false) {
        errors.geminiApiKey = apiKeyValidation.gemini.error || 'Invalid Gemini API key';
      }
    }

    // GitHub token validation (optional but if provided, must be valid)
    if (formData.githubToken && apiKeyValidation.github.isValid === false) {
      errors.githubToken = apiKeyValidation.github.error || 'Invalid GitHub token';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check if API keys are still being validated
    const isValidating = Object.values(apiKeyValidation).some(validation => validation.isValidating);
    if (isValidating) {
      setError('Please wait for API key validation to complete');
      return;
    }
  ...
  const handleLanguageChange = (language: string) => {
    updateFormData('preferredLanguage', language);
    updateFormData('preferredFrameworks', []); // Reset frameworks when language changes
  };
  ...
  const handleFrameworkToggle = (framework: string) => {
    ...
    updateFormData('preferredFrameworks', newFrameworks);
  };
      ...
      <div className="bg-gray-800 rounded-lg shadow-xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <button
              type="button"
              onClick={toggleApiKeysVisibility}
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`
                        block w-full px-3 py-3 pr-10 border rounded-lg
                        bg-gray-600 text-white placeholder-gray-400
                        focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent
                        ${validationErrors.openaiApiKey ? 'border-red-500' : 
                          apiKeyValidation.openai.isValid === true ? 'border-green-500' :
                          apiKeyValidation.openai.isValid === false ? 'border-red-500' : 'border-gray-500'}
                      `}
                      placeholder="sk-..."
                      disabled={isLoading}
                    />
                    <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                      {apiKeyValidation.openai.isValidating ? (
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`
                        ...
                        bg-gray-600 text-white placeholder-gray-400
                        focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent
                        ${validationErrors.geminiApiKey ? 'border-red-500' : 
                          apiKeyValidation.gemini.isValid === true ? 'border-green-500' :
                          apiKeyValidation.gemini.isValid === false ? 'border-red-500' : 'border-gray-500'}
                      `}
                      placeholder="Enter your Gemini API key"
                      disabled={isLoading}
                    />
                    <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                      {apiKeyValidation.gemini.isValidating ? (
                        <Loader2 className="h-4 w-4 text-blue-400 animate-spin" />
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`
                        ...
                        bg-gray-600 text-white placeholder-gray-400
                        focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent
                        ${validationErrors.githubToken ? 'border-red-500' : 
                          apiKeyValidation.github.isValid === true ? 'border-green-500' :
                          apiKeyValidation.github.isValid === false ? 'border-red-500' : 'border-gray-500'}
                      `}
                      placeholder="ghp_... or github_pat_..."
                      disabled={isLoading}
                    />
                    <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                      {apiKeyValidation.github.isValidating ? (
                        <Loader2 className="h-4 w-4 text-blue-400 animate-spin" />
                      ) : apiKeyValidation.github.isValid === true ? (
                        <CheckCircle className="h-4 w-4 text-green-400" />
                      ) : apiKeyValidation.github.isValid === false ? (
                        <AlertCircle className="h-4 w-4 text-red-400" />
                      ) : null}
                    </div>
                  </div>
